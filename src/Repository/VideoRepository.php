...
class VideoRepository extends ServiceEntityRepository
{
    ...
    public function getVideosRecomendadosAPartirDeVideo(array $id): array
    {
        ...
        where tc.id = :idTipoCategoria and v.id != :idVideo and v.activo = true and v.id_tipo_privacidad = 1 order by v.fecha desc limit 10;';
    ...
    public function getVideosListas(array $id): array
    {
        $conn = $this->getEntityManager()->getConnection();
        $idUsuario = $id["id"];
        $sql = 'select v.* from safatuber24.video v join safatuber24.video_lista_reproduccion vlr on v.id = vlr.id_video
           where vlr.id_lista_reproduccion = :id;';
        $resultSet = $conn->executeQuery($sql, ['id' => $idUsuario]);
        return $resultSet->fetchAllAssociative();
    }
    ...
    public function getVideosCanalesSuscritos(array $id): array
    {
        ...
        $sql = 'select v.*,  c.nombre as nombre_canal, c.foto as foto_canal from safatuber24.suscripcion s
                ...
                join safatuber24.tipo_privacidad tp on v.id_tipo_privacidad = tp.id
                where s.id_usuario_suscriptor = :id and (tp.id = 1 or tp.id = 3)  and v.activo = true
